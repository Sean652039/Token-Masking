...
def plot_selected_models_logits(all_models_logits, id2label, selected_probs=[0.0, 0.1]):
    """
    Plot logits distribution for only the selected mask probabilities (0.0 and 0.1)
    """
    # Filter the dictionary to only include selected probabilities
    selected_models_logits = {prob: all_models_logits[prob] for prob in selected_probs if prob in all_models_logits}

    # Compute consistent axis limits
    x_lim = compute_axis_limits(selected_models_logits)

    # 3D Plot
    fig_3d = plt.figure(figsize=(12, 5), dpi=300)
    plt.subplots_adjust(wspace=0.25, hspace=0.1)

    for i, (mask_prob, class_logits) in enumerate(selected_models_logits.items()):
        ax = fig_3d.add_subplot(1, 2, i + 1, projection='3d')
    ...
    plt.suptitle('3D Logits Distribution for Mask Probabilities 0.0 and 0.1', fontsize=16, fontweight='bold')
    plt.savefig('../plots/logits_distribution_3d_selected.png', bbox_inches='tight')
    plt.close()

    # 2D Plots for different perspectives
    perspectives = [
        {'dims': [0, 1], 'title': 'Dim 0 vs Dim 1'},
        {'dims': [1, 2], 'title': 'Dim 1 vs Dim 2'},
        {'dims': [0, 2], 'title': 'Dim 0 vs Dim 2'}  # Added this perspective
    ]

    for perspective in perspectives:
        fig_2d = plt.figure(figsize=(12, 5), dpi=300)
        plt.subplots_adjust(wspace=0.1, hspace=0.2)

        for i, (mask_prob, class_logits) in enumerate(selected_models_logits.items()):
            ax = fig_2d.add_subplot(1, 2, i + 1)
        ...
        plt.suptitle(f'2D Logits Distribution: {perspective["title"]} for Mask Probabilities 0.0 and 0.1', fontsize=16,
                     fontweight='bold')
        plt.savefig(f'../plots/logits_distribution_{x_dim}_{y_dim}_selected.png', bbox_inches='tight')
        plt.close()
...
def main():
    ...
    # We only need models with mask probability 0.0 and 0.1
    selected_models = model_dirs[:2]  # Assuming they are sorted and the first two are 0.0 and 0.1
    ...
    for model_dir in selected_models:
        ...
        # Load test dataset with no masking
        ...
        mask_prob += 0.1
        if mask_prob > 0.1:  # Only process up to 0.1
            break

    # Plot logits for selected models
    plot_selected_models_logits(all_models_logits, test_dataset.id2label, selected_probs=[0.0, 0.1])
